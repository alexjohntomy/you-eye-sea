...
function ProfessorDropdown({listOfProfessors} : dropdownTypes) {
    ...
    if (professorID == "all-professors" || !professorID) {
        ...
        professorNameFromID = (listOfProfessors.find(professor => professor.id === professorID)) ?? {name: "All Professors", id: "all-professors"}
    }

    return (
            <DropdownMenu>
            <DropdownMenuTrigger asChild>
            <Button variant="outline" className='h-10'>{professorNameFromID.name}<ChevronsUpDown/></Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent className="w-56">
            <DropdownMenuLabel>Professors</DropdownMenuLabel>
            <DropdownMenuSeparator />
            <DropdownMenuRadioGroup value={professorID} onValueChange={handleDropdownChange}>
                <DropdownMenuRadioItem value={"all-professors"}>All Professors</DropdownMenuRadioItem>
