...
async function createTableRowsFromData({subject, sortType, level} : FilterTypes) : Promise<rowDetails[]> {
  const courseSums : courseSumsType[] = await getGradeTotalsForSubject({subject, sortType, level})
  //Row Creation Step
  const allRows = 
  courseSums
  .filter(course => ((course._sum.A! + course._sum.B! + course._sum.C! + course._sum.D! + course._sum.F!) != 0))
  .map(item => 
  {
    ...
    let weightedGrades = ((item._sum.A ?? 0) * 4) + ((item._sum.B ?? 0) * 3) + ((item._sum.C ?? 0) * 2) + ((item._sum.D ?? 0) * 1)
    let gpa = (weightedGrades/totalGradeCount)
    let name = `${item.courseID} ${item.courseNumber}`
    let professor = item.professorID
    let passRate = ((((item._sum.A!) + (item._sum.B!) + (item._sum.C!) + (item._sum.D!))/(totalGradeCount))*100)
...
async function FilteredDataTableServer({subject, sortType, level} : FilterTypes){
