...
function getProfessorNameFromID(professorID: string, listOfProfessors : Professor[]) {
    let professorNameFromID : Professor = ({name: "General Rating", id: "all-professors"})

    if (professorID == "all-professors" || !professorID) {
        professorNameFromID = {name: "General Rating", id: "all-professors"}
    }
    else {
        professorNameFromID = (listOfProfessors.find((professor : Professor) => professor.id === professorID)) ?? {name: "General Rating", id: "all-professors"}
    }
        ...
        <div className='flex flex-col h-full justify-between'>
            ...
            <div className='py-6'>
                        <InputGroup className="overflow-y rounded-sm border-2 border-uic-navy-800/40 focus-visible:ring-0 bg-background">
                            <ReactRating className='relative top-2 bottom-2' style={{ maxWidth: 150 }} itemStyles={customStyles} value={rating} onChange={setRating} />
                            <InputGroupTextarea className="text-xs resize-none border-none shadow-none" placeholder="How was your experience in this class? Let others know..." value = {value} onChange={handleValueChange}/>
                            <InputGroupAddon align="block-end">
                                <InputGroupText className="ml-auto">{value.length}/1500</InputGroupText>
                                    <InputGroupButton
                                    variant="outline"
                ...
                <div className="flex flex-col w-full py-8 overflow-y-scroll gap-3">
                        ...
                        {reviews.map((eachReview : any) => (
                            ...
                            <ReactRating
                                ...
                                />                                
                                <h3 className="text-sm text-foreground/50 font-black border-b py-1">{getProfessorNameFromID(String(eachReview.professorID), listOfProfessors)}</h3>
                                ...
                                <span className="flex flex-row justify-between opacity-60">
                                    ...
                                    <h4 className="text-xs text-right inline-block text-foreground">{eachReview.date.toLocaleDateString('en-US')}</h4>
                                </span>
                            </Card>
                        ))}
                        </div>
